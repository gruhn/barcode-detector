...
struct scan_result {
    ...
    const char *data;
    const int *polygon;
    const unsigned polysize;
};

// External javascript function to pass the retrieved data to.
extern void js_output_result(const char *symbolName, const char *data, const int *polygon, const unsigned polysize, const struct scan_result scanResult);
...
int scan_image(uint8_t *raw, int width, int height)
{
    ...
    for (; symbol; symbol = zbar_symbol_next(symbol))
    {
        ...
        const struct scan_result scanResult = {
            zbar_get_symbol_name(typ), data, poly, poly_size
        };

        // Output the result to the javascript environment
        js_output_result(zbar_get_symbol_name(typ), data, poly, poly_size, scanResult);
    }
